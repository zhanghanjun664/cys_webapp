...
function drawCanvas(config){
  ...
  ctx.moveTo(config.box[0].x + paddingLeft, config.box[0].y);
  for(var i = 1;i<config.box.length;i++){
    ctx.lineTo(config.box[i].x + paddingLeft, config.box[i].y)
  }
  ...
  for(var i = 0;i<config.box.length;i++){
    ctx.beginPath();
    ctx.arc(config.box[i].x + paddingLeft, config.box[i].y,config.r,0,Math.PI*2);
