...
<head>
  ...
  <style>
    ...
    .saveDiv{width:94%;margin-left:3%;background:#FB9D3B;margin-top:10%; border-radius:0.2em;-webkit-border-radius:0.2em;text-align:center;font-size:1.8em;color:#FFFFFF;float:left;}
...
<div id="mainDiv">
  ...
  <ul class="btmul">
    <li class="btmul_li">
      <ul class="btmul_li_ul">
        ...
        <li class="btmul_li_ul_li2">
          <p class="btmul_li_ul_li2_p2">患者年龄：<font style="color:#A4A4A4;"><%=orderModel.getAge()%></font></p>
        </li>
        <li class="btmul_li_ul_li2">
          <p class="btmul_li_ul_li2_p3">联系手机：<font style="color:#A4A4A4;"><%=orderModel.getPhoneNumber()%></font></p>
        </li>
        <li class="btmul_li_ul_li1">
          <span class="btmul_li_ul_li1_sp1">诊断情况</span>
        </li>
        <li class="btmul_li_ul_li2">
          <p class="btmul_li_ul_li2_p1">诊断时间：<font style="color:#A4A4A4;"><%=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(availableTimeModel.getDateTime())%></font></p>
        </li>
        <li class="btmul_li_ul_li3" onClick="show1();">
          <p class="btmul_li_ul_li3_p2">病情类别</p>
          <p id="jd_sickcategory_label"></p>
        </li>
  ...
  <div class="saveDiv" onClick="saveCase();">保存</div>
</div>
<div id="classifyDiv">
  <ul class="classifyDiv_ul1">
    ...
    <%
      for(int i=0;i<sickCategaryQueryModelList.size();i++){
        ...
        %>
    <li class="classifyDiv_ul1_li1" alt="0" onClick="choose1('<%=sickCategaryQueryModel.getUuid()%>',this);"><span>
      <%=sickCategaryQueryModel.getName()%></span></li>
    <%
      }
    %>
  </ul>
...
<div id="situationDiv">
  <ul class="situationDiv_ul1">
    ...
    <li class="situationDiv_ul1_li2"><textarea id="situation_input"></textarea></li>
  </ul>
...
<script>
  ...
  window.onload = function(){
    ...
    $('.saveDiv').width(mw*0.94).height(mh*0.07);
    $('.saveDiv').css('line-height',mh*0.07+'px');
    ...
    $('#classifyDiv').width(mw).height(mh);
    ...
    $('#situationDiv').width(mw).height(mh);
  ...
  function saveCase(){
    if(sickcategory_id==null || sickcategory_id.length==0){
      MsgOpt.showMsgBox("请选择病情类别","确定");
      return;
    }
    ...
    data="sickcategory_id="+sickcategory_id+"&description="+description+"&orderUUID=<%=orderModel.getUuid()%>"
    $.ajax({
      type: 'POST',
      url: 'insertCase.htm',
      dataType: 'json',
      ...
      success: function (msg) {
        if(msg.result==1){
          MsgOpt.showMsgBox("病例创建成功","确定","editCase.htm?jd_casehistory_id="+msg.jd_casehistory_id);
        }else if(msg.result==3){
          window.location.href="toLogin.htm";
        }else{
          MsgOpt.showMsgBox("病例创建失败","确定");
        }
      },
      timeout: 10000,
      error: function () {
        var msg = '病例创建失败，请检查网络';
        var btnword = '确定';
