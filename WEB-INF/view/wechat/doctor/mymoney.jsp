...
<head>
  ...
  <style>
    ...
    #mainDiv .cardul{width:100%;overflow:hidden;margin-top:5%;}
    #mainDiv .cardul li{width:100%;float:left;margin-bottom:0.2em;background:url(../../wechat/images/icon6.png) center right no-repeat #FFFFFF;background-size:8% auto;}
    #mainDiv .cardul li span{font-size:1.6em;margin-left:3%;}
...
<div id="mainDiv">
  ...
  <ul class="moneyul">
    <li class="moneyul_li1">
      <p id="moneyul_li1_p1"><%=doctorModel.getAccountBalance()%></p>
  ...
  <ul class="cardul">
    <li onclick="window.location.href='addCard.htm'"><span><%=jdBankCardModel==null?"添加银行卡":jdBankCardModel.getBankName()%></span></li>
  </ul>
</div>
<div id="MsgBoxDiv">
  <ul>
...
<script>
  ...
  var isAddCardTip=<%=jdBankCardModel==null?"true":"false"%>;
  //跳转连接
  function jump(num){
    if(num==1){
      if(isAddCardTip==true){
        MsgOpt.showMsgBox("请先增加银行卡", "确定");
      }else {
        window.location.href = 'getcash.htm';
      }
    }
    if(num==2){
      window.location.href = 'getWithdrawalHistoryList.htm';
    }
  }
  //消息弹窗类
  var MsgOpt = {
