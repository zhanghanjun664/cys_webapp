...
<head>
  ...
  <style>
    ...
    #mainDiv .dateul li .cont_li2_span{width:30%;font-size:1.4em;color:#FB9D3B;padding:0.3em;text-align:center;border-radius:0.2em;-webkit-border-radius:0.2em;border:#FB9D3B 0.12em solid;-webkit-border:#FB9D3B 0.12em solid;margin-left:22%;}
...
<div id="mainDiv">
  ...
  <ul class="dateul">
    ...
    <li>
      ...
      <div class="cont_li2_span" onClick="trafficGuide();">详细路线</div>
    </li>
  </ul>
  <ul class="moneyul">
    <%
  ...
  <ul class="btnul">
    <li onClick="cancelOrder();">取消预约</li>
  </ul>
</div>
<div id="MsgBoxDiv">
  <ul>
...
<script>
  ...
  function trafficGuide(){
    window.location.href = "trafficGuide.htm?orderId=<%=order.getUuid()%>";
  }
  ...
  function cancelOrder(){
    if(!cancelFlg) {
      ...
      if(confirm(tip)){
        $.ajax({
          type:'GET',
          url:'deleteNotreatOrder.htm?orderId=<%=order.getUuid()%>',
          success:function(msg){
            if(msg=="success"){
              window.location.href = "toOrderList.htm?status=2";
