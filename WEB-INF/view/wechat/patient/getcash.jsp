...
<head>
  ...
  <style>
    ...
    #mainDiv .cont_ul li input[type="text"]{width:70%;font-size:1.55em;color:#000000;background:transparent;border:0;-webkit-border:0;margin-left:8%;float:left;}
    #mainDiv .cont_ul li:nth-child(1){margin-bottom:0.18em;font-size:1.7em;text-align:center;}
    #mainDiv .cont_ul li:nth-child(2){margin-bottom:0.18em;border-radius:0.5em;-webkit-border-radius:0.5em;background:#FFFFFF;}
...
<div id="mainDiv">
  ...
  <ul class="cont_ul">
    <li>可提现金额：<span id="moneysp"><%=patientModel.getAccountBalance()%></span></li>
    <li><input type="text" id="money" placeholder="输入提现的金额" /></li>
    <li onClick="getCash();">确定</li>
  </ul>
</div>
<div id="MsgBoxDiv">
  <ul>
...
<script>
  ...
  var mh = 1136*mw/640;
  var minWithdrawal = "<%=minWithdrawal%>";
  window.onload = function(){
    ...
    $('.cont_ul li').eq(0).css('line-height',mh*0.07+'px');
  ...
  function getCash(){
    ...
    var money = parseFloat($('#moneysp').text());
    if(ob > money){
      var msg = '提现金额不能大于可提现金额';
      var btnword = '确定';
      MsgOpt.showMsgBox(msg, btnword);
      return;
    } else if(ob < minWithdrawal) {
      ...
      $.ajax({
        ...
        success: function (msg) {
          if (msg) {
            ...
            MsgOpt.showMsgBox("提现成功，资金将于3天内发到您的微信钱包，请到时注意查收。", "确定", "toPersonal.htm");
          }
        },
        timeout: 10000,
        error: function () {
