var RongWebIMWidget;
console.log("进来了RongIMWidget.js");
...
(function(RongWebIMWidget) {
	...
	(function(conversation) {
		...
		var ConversationController = (function() {
			function ConversationController($scope, $http, $rootScope, conversationServer, WebIMWidget, conversationListServer, widgetConfig, providerdata, RongIMSDKServer) {
				this.$scope = $scope;
				this.$http = $http;
				...
				function uploadFileRefresh() {
					var uploadUrl = REST_PREFIX + "generalinfo/upload";
					var flow = new Flow({
						singleFile: true,
						target: uploadUrl,
						testChunks: false,
					...
					flow.assignBrowse(document.getElementById('flow-btn'));
					...
					flow.on('fileSuccess', function(file, message) {
						...
						var im = RongIMLib.ImageMessage.obtain(data.body.result.base64Code, data.body.result.resourceUrl);
						...
						console.log(RongWebIMWidget.Message.convert(content));
				...
				$scope.goPatientFile = function(id) {
					console.log("跳转到id是:" + id)
					var url = "/assets/views/patient-archive/patientArchiveEditor.html?id=" + 411354859;
		...
		var imagemessage = (function() {
			function imagemessage() {
				...
				this.template = '<div class="text-center">' +
					'<img style="cursor:zoom-in" ng-src="{{msg.content}}" data-image="{{msg.imageUri}}" ng-click="$root.showImg(msg.imageUri)"/> ' +
					'</div>';
			}
...
(function(RongWebIMWidget) {
	...
	(function(conversationlist) {
		var conversationListController = (function() {
			function conversationListController($scope, conversationListServer, WebIMWidget, $rootScope) {
				this.$scope = $scope;
				this.conversationListServer = conversationListServer;
				this.WebIMWidget = WebIMWidget;
...
(function(RongWebIMWidget) {
	...
	(function(conversationlist) {
		...
		var conversationItem = (function() {
			function conversationItem(conversationServer, conversationListServer, RongIMSDKServer, $rootScope) {
				//          	console.log(conversationServer,conversationListServer,RongIMSDKServer,$rootScope);
				this.conversationServer = conversationServer;
				this.conversationListServer = conversationListServer;
				this.RongIMSDKServer = RongIMSDKServer;
				this.restrict = "E";
				...
				RongIMLib.RongIMClient.getInstance().getConversationList({
					onSuccess: function(data) {
						...
						if(RongWebIMWidget.Helper.checkType(_this.providerdata.getUserInfo) == "function" && _this.theFirst) {
							console.log("拉用户信息");
							(function(a, b) {
								_this.providerdata.getUserInfo(a.map(function(item) {
...
(function(RongWebIMWidget) {
	...
	var eleConversationListWidth = 300,
		eleminbtnHeight = 50,
		eleminbtnWidth = 195,
		spacing = 3;
				...
				onChanged: function(status) {
					...
					switch(status) {
						...
						default:
							console.log(status);
							//                          WebIMWidget.init({
							//                          	appkey: _this.$rootScope.config.useAppKey,
							//								token: _this.$rootScope.config.token,
							//                          })
							break;
					}
				}
...
//					}
		...
		WebIMWidget.prototype.setConversation = function(targetType, targetId, title) {
			...
			//          this.conversationListServer.updateConversations().then(function () { });
		};
...
(function(RongWebIMWidget) {
	...
	var UserInfo = (function() {
		function UserInfo(userId, name, portraitUri, tel) {
			this.userId = userId;
			this.name = name;
	...
	var Conversation = (function() {
		//会话对象的属性
		function Conversation(targetType, targetId, title, tel, userId, patientArchiveCount) {
			this.targetType = targetType;
			this.targetId = targetId;
			this.title = title || "";
			this.tel = tel || "";
			this.userId = userId || "";
			this.patientArchiveCount = patientArchiveCount || "";
		}
		//为会话列表添加自定义属性
